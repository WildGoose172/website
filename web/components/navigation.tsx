...
export function Navigation({ nav }: NavigationProps) {
  ...
  return (
    <div className="border-glass-border bg-glass shadow-2xl/20 inset-shadow-sm inset-shadow-current/20 inset-shadow-sm container mx-auto mt-4 flex items-center justify-between gap-x-4 rounded-full border px-4 py-1.5 backdrop-blur-sm">
        ...
        <SheetContent side="top">
          ...
          <div className="flex flex-col gap-1 px-4">
            {nav?.links?.map(link => {
              if (link._type === 'navigationDropdown') {
                return (
                  <div key={link._key} className="flex flex-col gap-2 border-b">
                    <button
                      className={[
